...
{
    private $_columns = ['id', 'type', 'picture', 'thumb', 'link', 'description', 'content', 'created', 'created_by', 'modified', 'modified_by', 'status', 'ordering'];
    ...
    public function infoItem($arrParam,$options = null){
        if($options == null){
            $query[] = "SELECT `id`, `picture`, `status`, `ordering`";
    ...
    public function saveItem($arrParam, $options = null)
    {
        if($options['task'] == 'add'){
            ...
            $arrParam['form']['picture']    = $uploadObj->uploadFile($arrParam['form']['picture'],'banner');
    ...
    public function trashItems($arrParam,$options = [])
    {
        ...
        foreach($arrCheckboxAction as $v){
            ...
            $uploadObj->removeFile($arrParam['controller'],$arrImg[0]);
        }
    ...
    public function deleteItem($arrParam, $options = [])
    {
        ...
        $arrImg = $this->fetchAll($query);
        $uploadObj->removeFile('banner',$arrImg[0]);
